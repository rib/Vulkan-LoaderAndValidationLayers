...
#       NOTE:  The non-autoGenerated code is in the object_tracker.h header file
    ...
    def generate_procs(self):
        ...
        for o in vulkan.object_type_list:# vulkan.core.objects:
            ...
            procs_txt.append('    layer_data *my_data = get_my_data_ptr(get_dispatch_key(dispatchable_object), layer_data_map);')
            procs_txt.append('    my_data->%sMap[(uint64_t)vkObj] = pNewObjNode;' % (o))
            ...
            procs_txt.append('    layer_data *my_data = get_my_data_ptr(get_dispatch_key(dispatchable_object), layer_data_map);')
            procs_txt.append('    auto it = my_data->%sMap.find(object_handle);' % o)
            procs_txt.append('    if (it != my_data->%sMap.end()) {' % o)
            ...
            procs_txt.append('        my_data->%sMap.erase(it);' % (o))
        ...
        for do in sorted(obj_use_dict):
            ...
            procs_txt.append('        return VK_FALSE;')
            procs_txt.append('    layer_data *my_data = get_my_data_ptr(get_dispatch_key(dispatchable_object), layer_data_map);')
            procs_txt.append('    if (my_data->%sMap.find((uint64_t)object) == my_data->%sMap.end()) {' % (do, do))
            ...
            for o in sorted(obj_use_dict[do]):
                ...
                if o == "VkImage":
                    ...
                    procs_txt.append('    layer_data *my_data = get_my_data_ptr(get_dispatch_key(dispatchable_object), layer_data_map);')
                    procs_txt.append('    if ((my_data->%sMap.find((uint64_t)object) == my_data->%sMap.end()) &&' % (o, o))
    ...
    def generate_destroy_instance(self):
        ...
        gedi_txt.append('')
        gedi_txt.append('    for (auto iit = my_data->VkDeviceMap.begin(); iit != my_data->VkDeviceMap.end();) {')
        ...
        for o in vulkan.core.objects:
            if o in ['VkInstance', 'VkPhysicalDevice', 'VkQueue', 'VkDevice']:
                continue
            gedi_txt.append('            for (auto idt = my_data->%sMap.begin(); idt != my_data->%sMap.end();) {' % (o, o))
            gedi_txt.append('                OBJTRACK_NODE* pNode = idt->second;')
            gedi_txt.append('                if (pNode->belongsTo == iit->first) {')
            gedi_txt.append('                    log_msg(mid(instance), VK_DEBUG_REPORT_ERROR_BIT_EXT, pNode->objType, pNode->vkObj, __LINE__, OBJTRACK_OBJECT_LEAK, "OBJTRACK",')
            gedi_txt.append('                            "OBJ ERROR : %s object 0x%" PRIxLEAST64 " has not been destroyed.", string_VkDebugReportObjectTypeEXT(pNode->objType),')
            gedi_txt.append('                            pNode->vkObj);')
            gedi_txt.append('                    my_data->%sMap.erase(idt++);' % o )
            gedi_txt.append('                } else {')
            gedi_txt.append('                    ++idt;')
            gedi_txt.append('                }')
            gedi_txt.append('            }')
        gedi_txt.append('            my_data->VkDeviceMap.erase(iit++);')
        gedi_txt.append('        } else {')
        gedi_txt.append('            ++iit;')
        gedi_txt.append('        }')
        gedi_txt.append('    }')
    ...
    def generate_destroy_device(self):
        ...
        gedd_txt.append('    layer_data *my_data = get_my_data_ptr(key, layer_data_map);')
        for o in vulkan.core.objects:
            ...
            if o in ['VkInstance', 'VkPhysicalDevice', 'VkQueue', 'VkDevice', 'VkDescriptorSet', 'VkCommandBuffer']:
                continue
            gedd_txt.append('    for (auto it = my_data->%sMap.begin(); it != my_data->%sMap.end();) {' % (o, o))
            ...
            gedd_txt.append('            my_data->%sMap.erase(it++);' % o )
    ...
    def _dereference_conditionally(self, indent, prefix, type_name, name):
        ...
        if type_name == 'pBufferInfo':
            ...
            s_code += '%slayer_data *my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);\n'       % (indent)
            s_code += '%sOBJTRACK_NODE* pNode = my_data->VkCommandBufferMap[(uint64_t)commandBuffer];\n'       % (indent)
