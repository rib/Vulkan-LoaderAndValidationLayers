...
#define ERR_EXIT(err_msg, err_class)                                           \
    do {                                                                       \
        ((void)__android_log_print(ANDROID_LOG_INFO, APP_SHORT_NAME, err_msg));\
        exit(1);                                                               \
    } while (0)
#else
#define ERR_EXIT(err_msg, err_class)                                           \
...
static void demo_init(struct demo *demo, int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
       ...
       // Can't pass #define into a macro on Windows, just keep in sync
       assert(!strcmp(APP_SHORT_NAME, "cube"));
